...
describe("CodebuddySdkAgentRuntime", () => {
  ...
  it("passes uploaded images to the Codebuddy SDK as image content blocks", async () => {
    sdkMocks.query.mockReturnValue(
      streamMessages({
        type: "result",
        subtype: "success",
    ...
    const workspacePath = await mkdtemp(path.join(os.tmpdir(), "pets-agent-codebuddy-image-"));
